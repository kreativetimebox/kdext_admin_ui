...
// Display name for whoever a document is assigned to for HITL review — same
// join/expression as lib/bugTracker.js's HITL_ASSIGNED_TO_EXPR, duplicated
// here rather than shared per this codebase's existing convention.
const DOC_HITL_ASSIGNED_TO_NAME_EXPR = `COALESCE(NULLIF(BTRIM(CONCAT(hiu.first_name, ' ', hiu.last_name)), ''), hiu.email)`;
...
export async function getDocumentById(id) {
  const result = await query(
    `SELECT
       COALESCE(d.result_id, d.request_id)                            AS id,
       d.result_id                                                    AS result_id,
       ...
       d.hitl_status,
       d.hitl_assigned_to,
       ${DOC_HITL_ASSIGNED_TO_NAME_EXPR}                               AS hitl_assigned_to_name,
       d.fraud_risk_level,
       d.is_anomalous,
       d.is_duplicate,
  ...
  if (!row) return null;
  return { ...row, result_id: row.result_id ?? row.id ?? row.request_id };
}
 ...
 *
 * A row with an issue_type already set (from the Bug Tracking panel) only
 * actually becomes a tracked bug here, at publish time -- not the moment the
 * issue type was picked. That's what keeps a document out of the Bug Tracker
 * tab while it's still being reviewed/edited.
 * @param {string} id - result_id
  ...
  const result = await query(
    `UPDATE document_processing_requests
     SET hitl_updated_result = $1::json,
         ...
         updated_at = CURRENT_TIMESTAMP,
         -- Stamped once, the first publish after an issue_type is set; never
         -- overwritten on later republishes (COALESCE keeps the original).
         bug_flagged_at = COALESCE(bug_flagged_at, CASE WHEN issue_type IS NOT NULL THEN CURRENT_TIMESTAMP END),
         bug_tracker_id = COALESCE(bug_tracker_id, CASE WHEN issue_type IS NOT NULL THEN nextval('bug_tracker_id_seq') END)
     WHERE result_id = $2
     RETURNING result_id           AS id,
               request_id          AS request_id,
               user_id             AS user_id,
               ocr_document_type   AS ocr_document_type,
               client_document_type AS client_document_type,
               key_environment     AS key_environment,
               result_version,
...
export async function getDocumentsWithMissingFields(filters = {}) {
  ...
  if (keyEnvironment) {
    ...
    whereConditions.push(`LOWER(d.key_environment) = LOWER($${paramIndex})`);
    paramIndex++;
  }
...
/**
 * Write the manually-editable bug-tracking fields for one row. This only
 * ever stores the draft issue_type/issue_description/bug_status -- it does
 * NOT flag the row as a tracked bug. bug_flagged_at/bug_tracker_id are
 * assigned later, by updateHitlResult, the moment the document is actually
 * published; that's what keeps a document out of the Bug Tracker tab while
 * it's still being reviewed. bugStatus is only ever included in the SQL (and
 * hence only ever persisted) when the caller has already confirmed the
 * current user is SUPER_ADMIN -- callers that only pass
 * issueType/issueDescription never touch bug_status.
 */
export async function updateBugTracking(resultId, { issueType, issueDescription, bugStatus, actionStatus } = {}) {
  ...
  if (issueType !== undefined) {
    params.push(issueType);
    const itIdx = params.length;
    setClauses.push(`issue_type = $${itIdx}::text`);
  }
  if (issueDescription !== undefined) {
    params.push(issueDescription);
    setClauses.push(`issue_description = $${params.length}::text`);
  }
  if (bugStatus !== undefined) {
    params.push(bugStatus);
    setClauses.push(`bug_status = $${params.length}::text`);
  }
  if (actionStatus !== undefined) {
    params.push(actionStatus);
    setClauses.push(`action_status = $${params.length}::text`);
  }
  ...
  const result = await query(
    `UPDATE document_processing_requests
        SET ${setClauses.join(", ")}
      WHERE (result_id = $${params.length} OR request_id = $${params.length})
      RETURNING COALESCE(result_id, request_id) AS id, result_id, request_id, issue_type, issue_description, bug_status, action_status, bug_flagged_at, bug_tracker_id`,
    params
  );
  return result.rows[0] || null;
}
...
export async function addComment(resultId, { username, message }) {
  const result = await query(
    `UPDATE document_processing_requests
        ...
        updated_at = CURRENT_TIMESTAMP
      WHERE (result_id = $4 OR request_id = $4)
      RETURNING comments`,
 ...
 */
/**
 * @param {{ clientId?: number|string }} [options]
 */
export async function getFilterOptions({ clientId } = {}) {
  const scopeUserId = clientId ? Number(clientId) : null;
  ...
  const [usersResult, docTypesResult, keyEnvsResult] = await Promise.all([
    query(
      `SELECT
       ...
       WHERE (email IS NULL OR lower(email) <> lower($1))
         ${scopeUserId ? `AND user_id = ${scopeUserId}` : ""}
    ...
    query(
      `SELECT DISTINCT COALESCE(NULLIF(BTRIM(d.ocr_document_type), ''), dt.type_name) AS doc_type
       ...
       WHERE d.is_deleted = false
         AND (u.email IS NULL OR lower(u.email) <> lower($1))
         ${scopeUserId ? `AND d.user_id = ${scopeUserId}` : ""}
