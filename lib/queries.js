...
export async function updateBugTracking(resultId, { issueType, issueDescription, bugStatus } = {}) {
  ...
  if (issueType !== undefined) {
    ...
    // Explicit ::text cast: $itIdx is reused below in a bare `IS NOT NULL`
    // check with no column context to infer a type from, which Postgres
    // can't resolve on its own ("could not determine data type of parameter",
    // 42P08) even though the same param is typed here via the column assignment.
    ...
    // Assign a stable, human-friendly Bug Tracker ID the first time a bug is
    // flagged (issue_type goes NULL -> set), from a dedicated sequence. Never
    // reassigned once set. Same old-row-value semantics as bug_flagged_at.
    setClauses.push(`bug_tracker_id = COALESCE(bug_tracker_id, CASE WHEN issue_type IS NULL AND $${itIdx}::text IS NOT NULL THEN nextval('bug_tracker_id_seq') END)`);
  }
