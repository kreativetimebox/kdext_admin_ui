...
export async function getDocumentsWithMissingFields(filters = {}) {
  const { search = "", docType = "", showAll = false, clientId = "", businessName = "", status = "", keyEnvironment = "", bugStatus = "", issueType = "", sortBy = "", sortOrder = "desc", page = 1, pageSize = 50 } = filters;
...
export async function getDocumentsWithMissingFieldsForExport(filters = {}) {
  const { search = "", docType = "", showAll = true, clientId = "", businessName = "", status = "", keyEnvironment = "", bugStatus = "", issueType = "", sortBy = "", sortOrder = "desc" } = filters;
...
export async function updateBugTracking(resultId, { issueType, issueDescription, bugStatus } = {}) {
  ...
  const result = await query(
    `UPDATE document_processing_requests
      ...
      RETURNING result_id AS id, issue_type, issue_description, bug_status`,
    params
  );
  return result.rows[0] || null;
}
