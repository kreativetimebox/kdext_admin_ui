  ...
  const copied = await query(
    `UPDATE document_processing_requests old
        SET processing_result   = src.processing_result,
            ...
            hitl_updated_result = src.formatted_result,
            ocr_document_type   = $3,
            status              = 'COMPLETED',
            hitl_status         = 'TO_BE_TESTED',
            validation          = src.validation,
            result_version      = COALESCE(old.result_version, 1) + 1,
            completed_at        = CURRENT_TIMESTAMP,
            updated_at          = CURRENT_TIMESTAMP
       FROM document_processing_requests src
...
export async function getDocumentsWithMissingFields(filters = {}) {
  const { search = "", docType = "", showAll = false, clientId = "", businessName = "", status = "", keyEnvironment = "" } = filters;
  ...
  const sql = `
    SELECT
      ...
      submitted_at,
      updated_at,
      -- Keep validation consistent with the HITL status: anything that resolves
      -- to TO_BE_TESTED is reported as validation = false; only COMPLETED is true.
      (COALESCE(
        db_hitl_status,
        CASE
          WHEN validation = true
