...
/**
 * Update the hitl_updated_result column for a result row. This is the
 * human-corrected copy of the extraction; the original formatted_result is
 * left untouched.
 * @param {string} id - result_id
...
export async function getDocumentsWithMissingFields(filters = {}) {
  ...
  const sql = `
    SELECT
      ...
      ocr_document_type,
      CASE WHEN
      COALESCE(db_hitl_status,
        CASE
          WHEN hitl_assigned_to IS NOT NULL AND COALESCE(array_length(missing_fields, 1), 0) = 0
            THEN 'COMPLETED'
          WHEN hitl_assigned_to IS NOT NULL
            THEN 'PROCESSING'
          WHEN COALESCE(array_length(missing_fields, 1), 0) > 0
            THEN 'TO_BE_TESTED'
          ELSE 'PENDING'
        END
      ) = 'PENDING' AND db_hitl_check = 1 then 'TO_BE_TESTED' else 
               COALESCE(db_hitl_status,
                CASE
                  WHEN hitl_assigned_to IS NOT NULL AND COALESCE(array_length(missing_fields, 1), 0) = 0
                    THEN 'COMPLETED'
