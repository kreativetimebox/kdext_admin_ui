  ...
  const copied = await query(
    `UPDATE document_processing_requests old
        SET processing_result   = src.processing_result,
            ...
            validation          = CASE WHEN src.compute_status is null THEN null WHEN src.compute_status = 'COMPLETED' THEN true WHEN src.compute_status IN ('FAILED', 'PROCESSING', 'QUEUED') THEN null ELSE false END,
...
export async function getDocumentsWithMissingFields(filters = {}) {
  ...
  const sql = `
    SELECT
      ...
      WHEN compute_status IN ('FAILED', 'PROCESSING', 'QUEUED') THEN null
