  ...
  const result = await dexaiQuery(
    `SELECT
     ...
     WHERE iu.client_id = $1
       AND r.role_name IN ('CLIENT_USER', 'CLIENT')
       AND NOT EXISTS (
         SELECT 1 FROM user_roles ur2
         JOIN roles r2 ON r2.role_id = ur2.role_id
         WHERE ur2.internal_user_id = iu.internal_user_id AND r2.role_name = 'CLIENT_ADMIN'
       )
     GROUP BY iu.internal_user_id, iu.client_id, iu.email, iu.first_name, iu.last_name,
              iu.is_active, iu.created_at, iu.last_login_at, iu.page_access
...
/**
 * Every sub-user account across every client, for the SUPER_ADMIN Clients
 * tab's nested CLIENT_ADMIN → sub-user view (grouped client-side by
 * client_id).
 */
export async function getAllClientUsers() {
  const result = await dexaiQuery(
    `SELECT
     ...
     JOIN user_roles ur ON ur.internal_user_id = iu.internal_user_id
     JOIN roles r ON r.role_id = ur.role_id
     WHERE iu.client_id IS NOT NULL
       AND r.role_name IN ('CLIENT_USER', 'CLIENT')
       AND NOT EXISTS (
         SELECT 1 FROM user_roles ur2
         JOIN roles r2 ON r2.role_id = ur2.role_id
...
export async function createClientUser(clientId, { email, firstName, lastName, password, role = "CLIENT_USER", pageAccess = null }) {
  ...
  const access = pageAccess && typeof pageAccess === "object"
    ? {
        ...
        // hitlEdit is opt-in (default false) — a sub-user can only reach
        // /missing-fields if an admin explicitly flips this on.
        hitlEdit: pageAccess.hitlEdit === true,
      }
    : null;
  if (!email) {
...
/**
 * Update the page_access JSONB for a CLIENT sub-user. Merges the supplied
 * flags into the existing record (or starts from scratch) so callers only
 * need to send the keys they want to change. Scoped to clientId + CLIENT
 * role at the SQL level — a CLIENT_ADMIN can never touch another client's
 * users, and can only ever update CLIENT/CLIENT_USER accounts.
 * @param {number} internalUserId
 * @param {number} clientId
 * @param {{ dashboard?: boolean, businessAudit?: boolean, bugTracker?: boolean, hitlEdit?: boolean }} pageAccess
 */
export async function updateClientUserPageAccess(internalUserId, clientId, pageAccess) {
  // Normalise — only known boolean flags; unknown keys are stripped so the
  // stored JSON stays predictable and doesn't grow with stale garbage.
  const known = ["dashboard", "businessAudit", "bugTracker", "hitlEdit"];
  const sanitized = {};
  for (const k of known) {
    if (k in pageAccess) sanitized[k] = Boolean(pageAccess[k]);
  }

  const result = await dexaiQuery(
    `UPDATE internal_users iu
        SET page_access = COALESCE(page_access, '{}'::jsonb) || $3::jsonb,
    ...
    [internalUserId, clientId, JSON.stringify(sanitized)]
  );
  return result.rows[0] || null;
}

export async function deleteClientUser(internalUserId, clientId) {
