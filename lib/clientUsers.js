  ...
  const result = await dexaiQuery(
    `SELECT
     ...
     GROUP BY iu.internal_user_id, iu.email, iu.first_name, iu.last_name, iu.is_active,
              iu.created_at, iu.last_login_at
...
export async function createClientUser(clientId, { email, firstName, lastName, password }) {
  if (!email) {
  ...
  return dexaiTransaction(async (client) => {
    const inserted = await client.query(
      `INSERT INTO internal_users (email, first_name, last_name, password_hash, is_active, failed_login_attempts, client_id, client_password_enc)
       VALUES (LOWER($1), $2, $3, $4, true, 0, $5, $6)
       RETURNING internal_user_id, email, first_name, last_name, is_active, created_at, last_login_at, client_id`,
      [email, firstName || null, lastName || null, passwordHash, clientId, passwordEnc]
    );
    const user = inserted.rows[0];

    await client.query(
      `INSERT INTO user_roles (internal_user_id, role_id)
       SELECT $1, role_id FROM roles WHERE role_name = 'CLIENT_USER'`,
      [user.internal_user_id]
    );

    return { ...user, roles: ["CLIENT_USER"] };
  });
}
