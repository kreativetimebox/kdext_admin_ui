...
// Runs `callback(client)` inside a BEGIN/COMMIT, rolling back on any error.
// Use for multi-statement writes that must land atomically (e.g. creating a
// team member row plus its role assignments) — dexaiQuery alone only covers
// a single statement per checked-out client.
export async function dexaiTransaction(callback) {
