...
export async function getBugTrackerStats(filters = {}) {
  ...
  const sql = `
    SELECT
      ...
      COUNT(*) FILTER (WHERE d_action_status = 'Model Tuning')                                                AS model_tuning,
      COUNT(*) FILTER (WHERE d_action_status = 'Reprocessing')                                                AS reprocessing,
      COUNT(*) FILTER (WHERE d_action_status = 'Invalid Bad Image Closed')                                    AS invalid_bad_image_closed
    FROM (
      SELECT
  ...
  const byDocType = result.rows.map((r) => ({
    ...
    reprocessing: Number(r.reprocessing || 0),
    invalidBadImageClosed: Number(r.invalid_bad_image_closed || 0),
  }));
  ...
  const totals = byDocType.reduce(
    (acc, r) => {
      ...
      acc.reprocessing += r.reprocessing;
      acc.invalidBadImageClosed += r.invalidBadImageClosed;
      return acc;
    },
    { open: 0, toBeTested: 0, closed: 0, modelTuning: 0, reprocessing: 0, invalidBadImageClosed: 0 }
  );
