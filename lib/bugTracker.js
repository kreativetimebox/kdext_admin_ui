...
const BUG_TRACKER_SORT_COLUMNS = new Set([
  ...
  "ocr_document_type", "bug_status", "issue_type", "issue_description",
  "created_at", "hitl_assigned_to", "bug_flagged_at",
]);
...
// flagged shows as "no issue" instead of a blank cell. Filters (including
// the `d.issue_type IS NOT NULL` WHERE below) and bug_status's own
// derivation still read the raw d.issue_type column, not this — "no issue"
// is a label, not a real category a document can be filtered/counted
// against.
 ...
 * @param {object} filters - { search, companies[], docType, issueType, bugStatus, page, pageSize }
 */
export async function getBugTrackerRows(filters = {}) {
  const { search = "", companies = [], docType = "", issueType = "", bugStatus = "", sortBy = "", sortOrder = "desc", page = 1, pageSize = 50 } = filters;

  const whereConditions = [
    "d.is_deleted = false",
    "d.result_id IS NOT NULL",
    "d.issue_type IS NOT NULL",
  ];
  const params = [];
  let paramIndex = 1;
  ...
  if (companies.length) {
    ...
    whereConditions.push(`COALESCE(BTRIM(u.company_name), '') = ANY($${paramIndex}::text[])`);
    paramIndex++;
  }
...
export async function getBugTrackerRowsForExport(filters = {}) {
  const { search = "", companies = [], docType = "", issueType = "", bugStatus = "", sortBy = "", sortOrder = "desc", limit = EXPORT_ROW_CAP } = filters;
...
export async function getBugTrackerStats(filters = {}) {
  ...
  const sql = `
    SELECT
      ...
      COUNT(*) FILTER (WHERE compute_bug_status = 'TO_BE_TESTED')  AS to_be_tested,
    ...
    HAVING COUNT(*) FILTER (WHERE compute_bug_status IS NOT NULL) > 0
  ...
  const byDocType = result.rows.map((r) => ({
    ...
    open: Number(r.open),
    toBeTested: Number(r.to_be_tested),
