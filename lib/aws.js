...
async function locateObject(objectKey, preferredBucket) {
  ...
  for (const bucket of buckets) {
    ...
    if (status === "exists") return { bucket, client };
    if (status === "unknown" && !inconclusive) inconclusive = { bucket, client };
  }
...
export async function getSignedFileUrl(key, expiresIn = 3600) {
  ...
  const located = await locateObject(objectKey, preferred);
  if (!located) return null;
  const { bucket, client } = located;
