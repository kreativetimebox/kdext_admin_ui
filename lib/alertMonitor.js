...
// page reads. UI-only — no email notification.
//
...
// Two-tier thresholds: "warning" vs "critical" severity in the tab. Defaults
// follow the same 75%/90% split the Servers tab's Bar() already uses
// visually (amber above 60%, red above 85%).
 ...
 * per-row DB work (recordAlert upserts) so a 50-row batch doesn't fire 50
 * simultaneous queries against the shared MAIN_FINANCE_DB pool (max: 10) —
 * that would itself cause the latency spikes this background poller is
 * supposed to avoid inflicting on real user requests.
 */
...
/**
 * Upsert a detected problem by fingerprint — no email, UI-only. `message`
 * must be a stable, value-free description (e.g. "CPU usage high") so
 * repeated detections of the same underlying issue dedupe onto one row;
...
/** Resolve an alert (auto-resolve path — metric/container-down conditions clearing). */
async function resolveByFingerprint(fingerprint) {
  await dexaiQuery(
    "UPDATE monitor_alerts SET resolved_at = now() WHERE fingerprint = $1 AND resolved_at IS NULL",
    [fingerprint]
  );
}
...
async function checkContainerLogs(server, c) {
  ...
  // critical alert. This does NOT suppress the plain Python-style
...
async function checkDocumentFailures() {
  ...
  await mapWithConcurrency(result.rows, 8, (row) => {
 ...
 *
 * Severity is "warning" — this just means "still slow, keep an eye on it,"
 * not a confirmed failure. If it actually times out and the app marks it
 * status='FAILED', checkDocumentFailures picks that up separately as
 * critical.
 */
 ...
 *     (critical) immediately, regardless of how long it's been
...
async function reconcileStuckDocuments() {
  ...
  await mapWithConcurrency(result.rows, 8, async (row) => {
    const failed = row.status === "FAILED" || row.error_message || row.result_status === "error";
    const done = row.status === "COMPLETED";
    if (!failed && !done) return; // genuinely still stuck — leave it to checkStuckDocuments
...
/**
 * Re-checks every currently-active document_failed alert's document: once
 * the pipeline itself has completed the document without error (regardless
 * of whether the extracted result has quality issues still needing HITL
 * review), the failure that raised the alert no longer reflects reality, so
 * auto-resolve it instead of leaving a stale critical alert for someone to
 * notice and clear by hand.
 *
 * Deliberately checks the raw `status`/error columns, NOT hitl_status or
 * validation — this alert is about "did the pipeline run error out", not
 * "does a human still need to review the result". Mirrors the exact inverse
 * of checkDocumentFailures' raise condition (status = 'FAILED' OR
 * error_message IS NOT NULL OR formatted_result.status = 'error') so a
 * document can't simultaneously read as both freshly-alertable and resolved.
 */
async function reconcileFailedDocuments() {
  const active = await dexaiQuery(
    `SELECT DISTINCT container_name AS request_id
     FROM monitor_alerts
     WHERE category = 'document_failed' AND resolved_at IS NULL`
  );
  if (active.rows.length === 0) return;
  const requestIds = active.rows.map((r) => r.request_id);
  ...
  const result = await dexaiQuery(
    `SELECT request_id
     ...
     WHERE request_id = ANY($1::text[])
       ...
       AND formatted_result::jsonb->>'status' IS DISTINCT FROM 'error'`,
    [requestIds]
  );
  ...
  await Promise.all(
    watched.map(async (c) => {
      ...
      if (isContainerDown(c)) {
        ...
        return; // don't bother scanning logs of a dead container
      }
...
export async function runPollCycle() {
  ...
  const serverResults = await Promise.allSettled(servers.map((server) => pollServer(server)));
  serverResults.forEach((r, i) => {
    if (r.status === "rejected") {
      console.error(`[alert-monitor] poll failed for ${servers[i].name}:`, r.reason?.message || r.reason);
    }
  });

  const [failures, stuck, reconcileStuck, reconcileFailed] = await Promise.allSettled([
    checkDocumentFailures(),
    checkStuckDocuments(),
    reconcileStuckDocuments(),
    reconcileFailedDocuments(),
  ]);
  if (failures.status === "rejected") {
    console.error("[alert-monitor] document failure check failed:", failures.reason?.message || failures.reason);
  }
  if (stuck.status === "rejected") {
    console.error("[alert-monitor] stuck document check failed:", stuck.reason?.message || stuck.reason);
  }
  if (reconcileStuck.status === "rejected") {
    console.error("[alert-monitor] stuck document reconcile failed:", reconcileStuck.reason?.message || reconcileStuck.reason);
  }
  if (reconcileFailed.status === "rejected") {
    console.error("[alert-monitor] failed document reconcile failed:", reconcileFailed.reason?.message || reconcileFailed.reason);
  }
}
