...
export async function getDexaiUserResults(userId, filters = {}) {
  ...
  const result = await dexaiQuery(
    `SELECT
       ...
       COALESCE(
        ...
        CASE
          WHEN d.validation = true
            THEN 'COMPLETED'
          WHEN d.db_hitl_check = 0 AND COALESCE(array_length(d.missing_fields, 1), 0) = 0
            THEN 'COMPLETED'
          ELSE 'TO_BE_TESTED'
        END
      )                                                   AS status,
     ...
     FROM (SELECT *, COALESCE((formatted_result::jsonb->>'hitl_check')::int, 0) AS db_hitl_check FROM document_processing_requests) AS d
