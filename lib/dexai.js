...
export async function getDexaiUserResults(userId, filters = {}) {
  const { search = "", docType = "", status = "", keyEnvironment = "", bugStatus = "", issueType = "", page = 1, pageSize = 50 } = filters;
...
export async function getDexaiUserResultsForExport(userId, filters = {}) {
  const { search = "", docType = "", status = "", keyEnvironment = "", bugStatus = "", issueType = "" } = filters;
  ...
  const result = await dexaiQuery(
    `SELECT b.*, b.compute_status AS status, b.compute_bug_status AS bug_status
     ...
     ${whereClause}) b
     ${outerWhereClause}
     ORDER BY COALESCE(b.submitted_at, b.created_at) DESC NULLS LAST
