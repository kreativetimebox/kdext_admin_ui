...
const DEXAI_RESULTS_SORT_COLUMNS = new Set([
  ...
  "processing_duration_ms", "hitl_assigned_to", "is_anomalous", "is_duplicate", "fraud_risk_level",
]);
...
// expression as lib/bugTracker.js's HITL_ASSIGNED_TO_EXPR (duplicated per
// this codebase's existing convention rather than shared).
const HITL_ASSIGNED_TO_NAME_EXPR = `COALESCE(NULLIF(BTRIM(CONCAT(hiu.first_name, ' ', hiu.last_name)), ''), hiu.email)`;
...
export async function getDexaiUserResultsForExport(userId, filters = {}) {
  const { search = "", docType = "", status = "", keyEnvironment = "", bugStatus = "", issueType = "", sortBy = "", sortOrder = "desc", limit = EXPORT_ROW_CAP } = filters;
...
export async function getDexaiResultByRequestId(requestId) {
  const result = await dexaiQuery(
    `SELECT
       ...
       d.validation,
       d.hitl_assigned_to,
       ${HITL_ASSIGNED_TO_NAME_EXPR}                                   AS hitl_assigned_to_name,
       d.fraud_risk_level,
       d.is_anomalous,
       d.is_duplicate,
       ${ISSUE_TYPE_EXPR}                                             AS issue_type,
...
export async function getClients(filters = {}) {
  ...
  // u.last_login_at above is that OTHER product's own login timestamp (users
  // has its own password_hash/api_key for a separate, already-live system —
  // see lib/clientProvisioning.js's header comment). portal_* columns here
  // are this admin_ui's own CLIENT_ADMIN login, linked via internal_users.client_id.
  const result = await dexaiQuery(
    `SELECT
       u.user_id,
       u.first_name,
       ...
       u.company_name,
       COALESCE(stats.total_requests, 0)::int AS total_requests,
       iu.internal_user_id AS portal_internal_user_id,
       iu.is_active         AS portal_is_active,
       iu.last_login_at     AS portal_last_login_at
     FROM users u
     LEFT JOIN (
       SELECT
         user_id,
     ...
     LEFT JOIN internal_users iu
       ON iu.client_id = u.user_id
       -- internal_users.client_id is shared by the client's CLIENT_ADMIN row
       -- AND every CLIENT_USER they add (lib/clientUsers.js) — without this
       -- role check the join fans out and this client shows up once per
       -- CLIENT_USER account instead of once. Portal Login here means the
       -- CLIENT_ADMIN login specifically.
       AND EXISTS (
...
/**
 * Update editable fields on a single client (users) row. Only touches the
 * admin-facing profile fields — never password_hash/api_key/role/business_id
 * etc, which belong to the separate, already-live client product built on
 * this same table.
 */
export async function updateClient(userId, { email, firstName, lastName, companyName }) {
  const result = await dexaiQuery(
    `UPDATE users
      ...
      WHERE user_id = $1
      RETURNING user_id, email, first_name, last_name, company_name`,
    [userId, email || null, firstName || null, lastName || null, companyName || null]
  );
  return result.rows[0] || null;
}
...
export async function deleteClient(userId) {
  return dexaiTransaction(async (txClient) => {
    // 1. Delete user_roles for any internal_users belonging to this client
    ...
    // 2. Delete internal_users belonging to this client (portal admin + sub-users)
    await txClient.query(
      `DELETE FROM internal_users WHERE client_id = $1`,
      [userId]
    );

    // 3. Unlink client from document_processing_requests or keep documents intact with user_id = null
    await txClient.query(
      `UPDATE document_processing_requests SET user_id = NULL WHERE user_id = $1`,
      [userId]
    );

    // 4. Delete the client from users table
