...
export async function getDexaiUserResults(userId, filters = {}) {
  ...
  // status is a computed column (derived in the inner query below from
  // hitl_status/validation/error fields), so it can't be filtered via
  // whereConditions above — those run against the raw `d` row before the
  // COALESCE/CASE projection produces the displayed status. Filter it in an
  // outer WHERE against the inner subquery's own column name instead.
  ...
  const result = await dexaiQuery(
    ...
    from
    (SELECT b.*, b.compute_status AS status
     FROM (SELECT
       d.request_id,
       d.result_id,
       d.transaction_id,
     ...
     ${whereClause}) b
     ${outerWhereClause}
     ORDER BY COALESCE(b.submitted_at, b.created_at) DESC NULLS LAST) a
